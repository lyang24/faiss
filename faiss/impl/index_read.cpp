        ...
        if (h == fourcc("IHNr")) {
            ...
            FAISS_THROW_IF_NOT_MSG(
                    idxhnsw->metric_type == METRIC_L2 ||
                            idxhnsw->metric_type == METRIC_INNER_PRODUCT,
                    "IndexHNSWRaBitQ supports only L2 and inner product metrics");
            bool centered;
            bool has_build_storage;
            READ1_BOOL(centered);
            READ1_BOOL(has_build_storage);
            if (has_build_storage) {
                FAISS_THROW_IF_NOT_MSG(
                        idxhnsw->storage,
                        "mutable IndexHNSWRaBitQ requires compressed storage");
                std::unique_ptr<Index> build_storage(read_index(f, io_flags));
                auto* build_flat =
                        dynamic_cast<IndexFlat*>(build_storage.get());
                FAISS_THROW_IF_NOT_MSG(
                        build_flat,
                        "IndexHNSWRaBitQ build storage must be an IndexFlat");
                FAISS_THROW_IF_NOT_FMT(
                        build_flat->ntotal == idxhnsw->ntotal,
                        "HNSW build storage ntotal %" PRId64
                        " != index ntotal %" PRId64,
                        build_flat->ntotal,
                        idxhnsw->ntotal);
                FAISS_THROW_IF_NOT_FMT(
                        build_flat->d == idxhnsw->d,
                        "HNSW build storage d %d != index d %d",
                        build_flat->d,
                        idxhnsw->d);
                FAISS_THROW_IF_NOT_MSG(
                        build_flat->metric_type == idxhnsw->metric_type,
                        "HNSW build storage metric mismatch");
                idx_rabitq->build_storage = build_flat;
                build_storage.release();
            }
            if (idxhnsw->storage) {
                ...
                idxhnsw->hnsw.search_method = rq->rabitq.nb_bits >= 2
                        ? HNSW::SM_RABITQ
                        : HNSW::SM_DEFAULT;
            }
        }
