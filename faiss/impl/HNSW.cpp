...
float HNSW::diversity_prune_scale(const DistanceComputer& qdis) const {
    constexpr int kMinRaBitQM = 16;
    constexpr int kMaxRaBitQM = 32;
    constexpr float kRaBitQPruneAlpha = 1.05f;
    const auto* rabitq = dynamic_cast<const RaBitQDistanceComputer*>(&qdis);
    const int level0_capacity = nb_neighbors(0);

    // Relax diversity pruning only where the noisy 1-bit symmetric estimate
    // constructs a graph for staged 4-bit L2 search. Other bit widths,
    // narrow graphs, and wider graphs retain standard pruning until they are
    // independently validated at matched recall.
    if (rabitq == nullptr || rabitq->metric_type != METRIC_L2 ||
        rabitq->nb_bits != 4 || level0_capacity < 2 * kMinRaBitQM ||
        level0_capacity > 2 * kMaxRaBitQM) {
        return 1.0f;
    }
    return kRaBitQPruneAlpha * kRaBitQPruneAlpha;
}
 ...
 */
template <class Comp>
void HNSW::shrink_neighbor_list_scaled(
    ...
    while (input.size() > 0) {
        ...
        for (NodeDistFartherT<Comp> v2 : output) {
            ...
            if constexpr (Comp::is_max) {
                dist_v1_v2 *= pairwise_distance_scale;
            }
...
void HNSW::shrink_neighbor_list(
        ...
        size_t max_size,
        bool keep_max_size_level0) {
    shrink_neighbor_list_scaled<Comp>(
            qdis, input, output, max_size, keep_max_size_level0, 1.0f);
}
...
template void HNSW::shrink_neighbor_list<HNSW::C_similarity>(
        ...
        size_t,
        bool);
template void HNSW::shrink_neighbor_list_scaled<HNSW::C_distance>(
        DistanceComputer&,
        std::priority_queue<HNSW::NodeDistFartherT<HNSW::C_distance>>&,
        std::vector<HNSW::NodeDistFartherT<HNSW::C_distance>>&,
        size_t,
        bool,
        float);
template void HNSW::shrink_neighbor_list_scaled<HNSW::C_similarity>(
        DistanceComputer&,
        std::priority_queue<HNSW::NodeDistFartherT<HNSW::C_similarity>>&,
        std::vector<HNSW::NodeDistFartherT<HNSW::C_similarity>>&,
        size_t,
...
void shrink_neighbor_list_inner(
    ...
    HNSW::shrink_neighbor_list_scaled<C>(
            qdis,
            resultSet,
            returnlist,
            max_size,
            keep_max_size_level0,
            pairwise_distance_scale);
...
void HNSW::add_links_starting_from(
        ...
        VisitedTable& vt,
        bool keep_max_size_level0) {
    const float pairwise_distance_scale = diversity_prune_scale(ptdis);
    if (is_similarity) {
...
void HNSW::merge_reverse_links_deterministic(
        ...
        std::vector<storage_idx_t>& incoming,
        bool keep_max_size_level0) {
    const float pairwise_distance_scale = diversity_prune_scale(dis);
    if (is_similarity) {
