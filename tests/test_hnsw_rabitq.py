...
class TestHNSWRaBitQ(unittest.TestCase):
    ...
    def make_index(self, nb_bits=3, M=8):
        xt, xb, xq = self.make_data()
        index = faiss.IndexHNSWRaBitQ(
            xt.shape[1], M, nb_bits, faiss.METRIC_L2
        )
    ...
    def test_io_and_reset(self):
        index, xb, xq = self.make_index(nb_bits=4, M=16)
        cloned = faiss.clone_index(index)
        cloned.add(xb[:1])
        self.assertEqual(cloned.ntotal, len(xb) + 1)
    ...
    def test_deterministic_build(self):
        ...
        try:
            ...
            for threads in (1, 4):
                ...
                built.append(self.make_index(nb_bits=4, M=16)[0])

            np.testing.assert_array_equal(
                faiss.vector_to_array(built[0].hnsw.neighbors),
                faiss.vector_to_array(built[1].hnsw.neighbors),
            )
            np.testing.assert_array_equal(
                faiss.vector_to_array(built[0].hnsw.offsets),
                faiss.vector_to_array(built[1].hnsw.offsets),
            )
        finally:
